...
while IFS= read -r line; do
  ...
  if [ $i -eq $random_location ]; then	
	...
	echo $level_HASH | base64 | tr -d "\r\n" | cut -c 1-10 >> /home/$levelToBuild/$output_file
  fi
  i=$(($i + 1))
done < "$input_file"
...
#echo $level_HASH | base64 | tr -d "\r\n" | cut -c 1-10 > "$random_directory/$levelToBuild.password"

cd /home/$levelToBuild

## Create the README.txt file
levelinstructions="The flag for this level is stored in the file my_psswd next to the word millionth. Use the grep command to filter by the word millionth."
